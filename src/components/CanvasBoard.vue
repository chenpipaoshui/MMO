...
import { clampPointToImage, findLineHit, findNearestPointHit } from '../core/hitTest'
import {
  clampZoom,
  createViewport,
  fitImageToCanvas,
...
function isPointHighlighted(lineId: string, pointIndex: number): boolean {
  if (dragTarget.value?.lineId === lineId && dragTarget.value.pointIndex === pointIndex) return true
  if (hoverPoint.value?.lineId === lineId && hoverPoint.value.pointIndex === pointIndex) return true
  return false
}
...
function drawLines(ctx: CanvasRenderingContext2D, vp: Viewport) {
  ...
  for (const line of unselected) {
    drawPolyline(ctx, vp, line.points, line.color, line.id, line.strokeWidth, false)
  }
  for (const line of selected) {
    drawPolyline(ctx, vp, line.points, line.color, line.id, line.strokeWidth, true)
  }
}
...
function drawDraft(ctx: CanvasRenderingContext2D, vp: Viewport) {
  ...
  drawPolyline(ctx, vp, points, '#2563eb', '__draft__', DEFAULT_LINE_STROKE_WIDTH, true)
  ...
  if (points.length >= 1) {
    ...
    const realLen = store.metersPerPixel !== null ? len * store.metersPerPixel : null
...
function getExportDisplayRegion(): ImageRegion | null {
  ...
  if (!store.exportSelecting || !exportRegionStart.value || !exportRegionEnd.value || !store.image) {
    return null
  }
...
function drawExportRegionOverlay(ctx: CanvasRenderingContext2D, vp: Viewport) {
  ...
  const p2 = imageToScreen({ x: region.x + region.width, y: region.y + region.height }, vp)
  ...
  if (confirmedExportRegion.value) {
    ...
    for (const pt of corners) {
      ...
      ctx.strokeRect(s.x - handleSize / 2, s.y - handleSize / 2, handleSize, handleSize)
    }
  }
  ctx.restore()
}
...
function updateExportCornerHover(screenPoint: Point) {
  if (!store.exportSelecting || !confirmedExportRegion.value || draggingExportCorner.value) {
...
function fitImage() {
  ...
  viewport.value = fitImageToCanvas(img.width, img.height, canvasSize.value.width, canvasSize.value.height)
  draw()
}
...
function updateHoverPoint(screenPoint: Point) {
  if (store.activeTool !== 'select' || isDraggingPoint.value || !store.image || !store.selectedLineId) {
    hoverPoint.value = null
    return
  }
  const hit = findNearestPointHit(screenPoint, selectableLines(), viewport.value, store.selectedLineId)
  hoverPoint.value = hit ? { lineId: hit.lineId, pointIndex: hit.pointIndex } : null
}
...
function onMouseDown(event: MouseEvent) {
  ...
  if (store.activeTool === 'select' && event.button === 0) {
    const pointHit = findNearestPointHit(point, selectableLines(), viewport.value, store.selectedLineId)
    if (pointHit) {
      store.selectLine(pointHit.lineId)
      isDraggingPoint.value = true
      dragTarget.value = { lineId: pointHit.lineId, pointIndex: pointHit.pointIndex }
      event.preventDefault()
      bindWindowPointerHandlers()
      return
    }
...
function handlePointerMove(event: MouseEvent) {
  ...
  if (draggingExportCorner.value && confirmedExportRegion.value && store.image) {
    const imagePoint = clampPointToImage(
  ...
  if (isDraggingPoint.value && dragTarget.value && store.image) {
    ...
    store.updateLinePoint(dragTarget.value.lineId, dragTarget.value.pointIndex, clamped)
    draw()
    return
  }
...
function endPointerInteraction() {
  ...
  if (wasExportDragging && exportRegionStart.value && exportRegionEnd.value && store.image) {
...
function onMouseMove(event: MouseEvent) {
  if (isDraggingPoint.value || isPanning.value || isDraggingExportRegion.value || draggingExportCorner.value) {
    return
  }
  lastMouse.value = getCanvasPoint(event)
...
watch(
  () => [store.lines, store.selectedLineId, store.draftPoints, store.image, store.activeTool],
  () => draw(),
  { deep: true },
)
...
<template>
  <div
    ...
    :class="[
      isPanning || spacePressed || store.activeTool === 'pan' ? 'cursor-grabbing' : '',
      store.activeTool === 'polyline' && !spacePressed ? 'cursor-crosshair' : '',
      store.activeTool === 'pan' && !isPanning ? 'cursor-grab' : '',
      store.activeTool === 'select' && isDraggingPoint ? 'cursor-grabbing' : '',
      store.activeTool === 'select' && !isDraggingPoint && hoverPoint ? 'cursor-grab' : '',
    ...
    :style="
      store.exportSelecting && (draggingExportCorner || hoverExportCorner)
        ? { cursor: exportCornerCursor(draggingExportCorner || hoverExportCorner) }
        : undefined
    "
  >
    <canvas
    ...
    >
      <span class="text-xs text-slate-500">
        已选 {{ confirmedExportRegion.width }} × {{ confirmedExportRegion.height }} px
      </span>
      <button
        type="button"
