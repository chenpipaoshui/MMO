...
export function getExportRegionCorners(
  region: ImageRegion,
): Record<ExportRegionCorner, Point> {
  return {
...
export function hitExportRegionCorner(
  ...
  for (const [key, pt] of Object.entries(corners) as [
    ExportRegionCorner,
    Point,
  ][]) {
  ...
  return new Promise((resolve, reject) => {
    canvas.toBlob(
      (blob) => {
        ...
        link.href = url;
        link.download = filename.endsWith(".jpg")
          ? filename
          : `${filename}.jpg`;
        link.click();
