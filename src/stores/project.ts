...
import {
  ...
  DEFAULT_MAP_SCALE_DENOMINATOR,
  MAX_LABEL_FONT_SIZE,
  MIN_LABEL_FONT_SIZE,
  MAX_LINE_STROKE_WIDTH,
  MIN_LINE_STROKE_WIDTH,
} from "../types";
...
function createLine(
  ...
  return {
    ...
    realLengthMeters:
      metersPerPixel !== null
        ? pixelToRealLength(pixelLength, metersPerPixel)
        : null,
    visible: true,
...
export const useProjectStore = defineStore("project", () => {
  ...
  const paletteColors = computed(() =>
    mergePaletteColors(PRESET_LINE_COLORS, customColors.value),
  );
  ...
  const metersPerPixel = computed(() => {
    ...
    if (
      !img ||
      paperSizeId.value === null ||
      mapScaleDenominator.value === null
    )
      return null;
  ...
  const totalRealLength = computed(() => {
    const valid = lines.value.filter(
      (l) => l.visible && l.realLengthMeters !== null,
    );
  ...
  function recalculateAllLines() {
    ...
    lines.value = lines.value.map((line) => {
      ...
      return {
        ...
        pixelLength,
        realLengthMeters:
          mpp !== null ? pixelToRealLength(pixelLength, mpp) : null,
      };
    });
  }

  function loadMap(file: File, sizeId: PaperSizeId, pdfPage = 1) {
    return loadMapFileAsImage(file, pdfPage, sizeId).then((img) => {
      image.value = img;
  ...
  function finishDraft() {
    ...
    const line = createLine(
      draftPoints.value,
      nextLineColor(lines.value.length),
  ...
  function updateLineLabelFontSize(lineId: string, fontSize: number) {
    const size = Math.min(
      MAX_LABEL_FONT_SIZE,
      Math.max(MIN_LABEL_FONT_SIZE, Math.round(fontSize)),
    );
    lines.value = lines.value.map((line) =>
  ...
  function updateLineStrokeWidth(lineId: string, width: number) {
    const strokeWidth = Math.min(
      MAX_LINE_STROKE_WIDTH,
      Math.max(MIN_LINE_STROKE_WIDTH, Math.round(width)),
    );
    lines.value = lines.value.map((line) =>
